...
//'
//' @param string A character string or a character vector where
//'   match is sought.
...
//' 
// [[Rcpp::export]]
SEXP re2_match_cpp(StringVector string, SEXP pattern,
  ...
  static auto anchor_enum = [] (const char* val) {
    if (strcmp(val, "UNANCHORED") == 0) {
      return RE2::UNANCHORED;
    }
    if (strcmp(val, "ANCHOR_START") == 0) {
      return RE2::ANCHOR_START;
    }
    if (strcmp(val, "ANCHOR_BOTH") == 0) {
...
#define NAME_IS_EQUAL(name) (strcmp(R_CHAR(names(i)), #name) == 0)
  ...
  if (more_options.isNotNull()) {
	...
	if (NAME_IS_EQUAL(logical) || NAME_IS_EQUAL(l)) {
	  ...
	  anchor = anchor_enum(R_CHAR(mopts(i)));
	} else {
	  const char* fmt
  ...
  if (nsubmatch < 0) {
    nsubmatch = re2proxy.get().NumberOfCapturingGroups() + 1;
  }

  return re2_match_inner(string, re2proxy.get(), logical, verbose,
			 startpos, endpos, nsubmatch, anchor);
}
...
SEXP re2_match_inner(StringVector text, const RE2& re2regexp,
  ...
  bool endpos_set = (endpos != SIZE_MAX);
  size_t _endpos;
  ...
  for (int i = 0; i < text.size(); i++) {
    ...
    _endpos = endpos_set ? std::min(endpos, strp.size()) : strp.size();

    if (nsubmatch == 0) {
      lv[i] = re2regexp.Match(strp, startpos, _endpos, anchor,
			      nullptr, 0);
    } else {
      lv[i] = re2regexp.Match(strp, startpos, _endpos, anchor,
