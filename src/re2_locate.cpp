...
namespace {
  struct DoLocate : re2::DoMatchIntf {
	...
	result(i, 0) = static_cast<size_t>(sp_arr[0].begin()
					   - text.begin() + 1);
	result(i, 1) = static_cast<size_t>(sp_arr[0].end()
					   - text.begin());
      }
    }
...
namespace {
  struct DoLocateAll : re2::DoMatchIntf {
	...
	if (sp_arr[0].data() == NULL) {
	  ...
	  mat(row, 0) = static_cast<size_t>(sp_arr[0].begin()
					    - text.begin() + 1);
	  mat(row, 1) = static_cast<size_t>(sp_arr[0].end()
					    - text.begin());
	}
      }
      result[i] = mat;	
